...
const DATA_EPG_PATH =           path.join(DATA_PATH, 'epg');
...
function saveChannelEPG(segmentId, timestamp, epg, data, channels, callback) {
  var channel = searchChannelBySegmentId(segmentId);
  fs.writeFile(path.join(DATA_EPG_PATH, timestamp.toString(), channel.toString()), JSON.stringify(epg), function (err) {
    if (err) {
      log.error('[EPG] Could not save epg for channel ' + channel + ': ' + err);
    }
    parseEPG(timestamp, data, channels, callback);
  });
}

function parseEPG(timestamp, data, channels, callback) {
  process.stdout.write("Remaining " + channels.length + " channels\r");
  ...
  if (pendingItems == 0) {
    saveChannelEPG(segmentId, timestamp, epg, data, channels, callback);
    return;
  }
...
function getEPG(timestamp, days) {
  ...
  getMulticastData(mcast_epg_group, EPG_PORT, segmentIds, function(data) {
    ...
    log.debug('[EPG] Parse epg data');
      parseEPG(epgTimestamp, data, Object.keys(data), function() {
...
function parseSubscribedChannels(xml) {
  ...
  json = xml2json.toJson(xml[CHANNELS_DESCRIPTION_ID]);
...
function getMulticastData(mcast_group, mcast_port, payloadIDs, callback) {
  ...
  var doRun = function (message, remote) {
    counter++;
    process.stdout.write("Received " + counter + " packets\r");
...
function initialize() {
  ...
  try {
    ...
    channelMetadatas = require('./data/channels_metadata.json')
  } catch (e) {
